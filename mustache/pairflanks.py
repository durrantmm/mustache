import sys
import click
from mustache import misc, sctools, flanktrie
import pysam
...
def get_softclipped_sites(bam_file, min_softclip_length=4, min_softclip_count=5, min_alignment_quality=20):
    ...
    soft_clips = pd.DataFrame.from_dict(soft_clips, orient='index', columns=column_names)
...
def determine_flank_sequence(softclipped_sites, bam_file, min_softclip_length=4, min_softclip_count=5, min_alignment_quality=20):
    ...
    for index, row in softclipped_sites.iterrows():
        ...
        if site_count % 1000 == 0:
            logger.info("\tAfter processing %d sites, %d unique candidates found..." % (site_count, len(final_flank_seqs)))
            ...
            for pr in pu.pileups:
                ...
                if orient == 'L':
                    if sctools.is_left_softclipped_lenient_at_site(read, contig, pos):
                        softclip_count += 1
                        softclipped_seqs.append(sctools.left_softclipped_sequence(read))
                        softclipped_qualities.append(sctools.left_softclip_qualities(read))

                elif orient == 'R':
                    if sctools.is_right_softclipped_lenient_at_site(read, contig, pos):
                        softclip_count += 1
                        softclipped_seqs.append(sctools.right_softclipped_sequence(read))
                        softclipped_qualities.append(sctools.right_softclip_qualities(read))
    ...
    final_flank_seqs = pd.DataFrame.from_dict(final_flank_seqs, orient='index', columns=column_names)
...
@click.option('--output_file', '-o', default='mustache.flanks.tsv', help="The output file to save the results.")
...
@click.option('--min_softclip_count', '-mincount', default=5, help="For a softclipped site to be considered, there must be at least this many softclipped reads at the site.")
...
def findflanks(bamfile, min_softclip_length, min_softclip_count, min_alignment_quality, output_file=None):
    ...
    logger.info("Determining flank sequences from softclipped reads...")
    flank_sequences = determine_flank_sequence(softclipped_sites, bam, min_softclip_length, min_softclip_count, min_alignment_quality)
