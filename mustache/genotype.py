...
def _genotype(clusterseq, pairflankfiles, filter_clusters_inferred_assembly, output_file):


    logger.info("Loading clusterseq file...")
    clusterseq = pd.read_table(clusterseq)[['sample', 'pair_id', 'method', 'seqid', 'cluster', 'group']]

    logger.info("Parsing pairflank files")
    if len(pairflankfiles) == 1 and is_path_list(pairflankfiles[0]):
        pairflankfiles = [l.strip() for l in open(pairflankfiles[0], 'r')]

    logger.info("Loading pairflank files...")
    pairflanks = combine_pairflank_files(pairflankfiles)


    genotyper = Genotyper(clusterseq, pairflanks, filter_clusters_inferred_assembly)


    if pairflanks.shape[0] == 0:
...
def combine_pairflank_files(pairflankfiles):

    keep_header = ['sample', 'pair_id', 'contig', 'pos_5p', 'pos_3p']

    click.echo('Loading file {num1}/{num2}: {f}'.format(num1=1, num2=len(pairflankfiles), f=pairflankfiles[0]))

    f0 = pairflankfiles[0]
    pairflanks = pd.read_table(f0)[keep_header]


    for i, f in enumerate(pairflankfiles[1:]):
        click.echo('Loading file {num1}/{num2}: {f}'.format(num1=i + 2, num2=len(pairflankfiles), f=f))

        df = pd.read_table(f)[keep_header]
        pairflanks = pairflanks.append(df)
...
class Genotyper:
    ...
    def __init__(self, clusterseq, pairflanks, filter_clusters_inferred_assembly=True):

        self.clusterseq = clusterseq
        self.pairflanks = pairflanks
        self.filter_clusters_inferred_assembly = filter_clusters_inferred_assembly

        self.pairflanks['pair_id'] = list(map(int, list(self.pairflanks['pair_id'])))
        self.pairflanks['pos_5p'] = list(map(int, list(self.pairflanks['pos_5p'])))
        self.pairflanks['pos_3p'] = list(map(int, list(self.pairflanks['pos_3p'])))
    ...
    def genotype(self):
        ...
        combined = pd.merge(self.pairflanks, self.clusterseq, how='left', on=['sample', 'pair_id'])
