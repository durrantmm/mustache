...
def workflow(workdir, snakefile, config, cores, memory, unlock):
    ...
    log_params(command='workflow', workdir=workdir, snakefile=snakefile, config=config, cores=cores, memory=memory,
                 unlock=unlock)
    _workflow(workdir, snakefile, config, cores, memory, unlock)
...
@cli.command(short_help='Find insertion sites and reconstruct flanks of inserted sequence', help_priority=3)
...
@cli.command(short_help="Pair identified flanks with each other to represent 5' and 3' ends of inserted sequence.", help_priority=4)
...
def pairflanks(flanksfile, bamfile, genome, max_direct_repeat_length, min_alignment_quality,
    ...
    log_params(command='pairflanks', flanksfile=flanksfile, bamfile=bamfile, genome=genome,
...
@cli.command(short_help='Infers the identity of an inserted sequence by aligning flank pairs to an assembled genome.', help_priority=5)
...
@click.argument('pairflankfiles', type=click.Path(exists=True), nargs=-1)
...
def genotype(clusterseq, pairflankfiles, filter_clusters_inferred_assembly, output_file):
    log_params(command='genotype', clusterseq=clusterseq, pairflankfiles=pairflankfiles,
               filter_clusters_inferred_assembly=filter_clusters_inferred_assembly,
               output_file=output_file)
    _genotype(clusterseq, pairflankfiles, filter_clusters_inferred_assembly, output_file)
...
@cli.command(short_help='Recall softclip counts and runthrough counts from BAM file at specified pairflank insertions.', help_priority=15)
@click.argument('pairsfile', type=click.Path(exists=True))
@click.argument('bamfile', type=click.Path(exists=True))
@click.option('--min_alignment_quality', '-minq', default=20, help="For a read to be considered, it must meet this alignment quality cutoff. default=20")
@click.option('--min_alignment_inner_length', '-minial', default=21, help="If a read is softclipped on both ends, the aligned portion must be at least this long. Ideally, set this equal to 1 + maximum direct repeat length. default=21")
@click.option('--large_insertion_cutoff', '-lins', default=30, help="Same parameter as that used for findflanks.")
