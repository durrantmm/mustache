...
def single_end_pipeline(bamfile, output_prefix, min_softclip_length, min_softclip_count, min_alignment_quality, blastdb,
    ...
    flanks_output_file = output_prefix + '.findflanks.tsv'
    if findflanks:
        _findflanks(bamfile, min_softclip_length, min_softclip_count, min_alignment_quality, flanks_output_file)
    logger.info('')

    logger.info("BEGINNING PAIRFLANKS...")
    pairs_output_file = output_prefix + '.pairflanks.tsv'
    _pairflanks(flanks_output_file, pairs_output_file)
    logger.info('')
