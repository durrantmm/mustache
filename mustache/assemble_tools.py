...
class Minimus2Merger:
    ...
    def delete_files(self):
        shell('rm -rf {reads} {afg} {outprefix}.bnk {outprefix}.fasta '
              '{outprefix}.coords {outprefix}.fasta {outprefix}.ovl {outprefix}.OVL '
    ...
    def merge(self, ignore_overhangs=2, min_overlap_length=10):
        ...
        shell("toAmos -s {reads_path} -o {afg_path} 2> junk;".format(reads_path=self.reads_path, afg_path=self.afg_path),read=True)
        shell("bank-transact -f -z -b {outprefix}.bnk -m {afg_path} 2> junk;".format(outprefix=self.full_outprefix, afg_path=self.afg_path), read=True)
        shell("dumpreads {outprefix}.bnk -M 1 > {outprefix}.ref.seq 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("dumpreads {outprefix}.bnk -m 1 > {outprefix}.qry.seq 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("nucmer -maxmatch -c {min_olap} {outprefix}.ref.seq {outprefix}.qry.seq -p {outprefix} 2> junk;".format(outprefix=self.full_outprefix, min_olap=min_overlap_length), read=True)

        # Will fail if there are no overlaps
        try:
            shell("show-coords -H -c -l -o -r -I 94 {outprefix}.delta | nucmerAnnotate | egrep 'BEGIN|END|CONTAIN|IDENTITY' > {outprefix}.coords 2> junk;".format(outprefix=self.full_outprefix))
        except subprocess.CalledProcessError:
            shell("rm -f junk;")
            return None

        shell("nucmer2ovl -ignore {ignore} -tab {outprefix}.coords | sort2 > {outprefix}.ovl 2> junk;".format(outprefix=self.full_outprefix, ignore=ignore_overhangs), read=True)
        shell("ovl2OVL {outprefix}.ovl  > {outprefix}.OVL 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("bank-transact -z -b {outprefix}.bnk -m {outprefix}.OVL 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("tigger -b {outprefix}.bnk 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("make-consensus -B -e 0.06 -b {outprefix}.bnk -w 15 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell("bank2fasta -b {outprefix}.bnk > {outprefix}.fasta 2> junk;".format(outprefix=self.full_outprefix), read=True)
        shell('rm -f junk;')
